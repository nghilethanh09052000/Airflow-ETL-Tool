...
def airflow_callback(context):
    ...
    try:
        ...
        if clickup_task["status"]["status"].lower() in ["done", "resolved", "skip"]:
            logging.info("Error was resolved in the past but occurring again")
            new_task = create_click_up_task(error_info).json()
            update_clickup_info_to_error_info(new_task, error_info)
            send_error_data(error_info)
            ...
            logging.info("Ticket is being processed, snoozing error notification")

    except requests.exceptions.HTTPError as e:
        if e.response.status_code == 404:
            logging.info("Task does not exist. Creating a new task.")
            new_task = create_click_up_task(error_info).json()
            update_clickup_info_to_error_info(new_task, error_info)
            send_error_data(error_info)
        else:
...
def get_dag_info(context) -> Dict:
    ...
    """
        Update 2024-05-29 Because we use VM so it always come our hosting to localhost,
        So we hardcode the airflow_url instead of using regex
    
    """    
    if Variable.get("execution_environment") == "production":
        dag_info["task_log_url"] = re.sub(r"^http://localhost:8080", "http://34.45.100.230:8080", dag_info["task_log_url"])
    
    
    pattern = r"^(http?://[^/]+)"
    dag_info["airflow_url"] = re.search(pattern, dag_info["task_log_url"]).group(1)  
    dag_info["dag_tree_url"] = f"{dag_info['airflow_url']}/tree?dag_id={dag_info['dag_id']}"
